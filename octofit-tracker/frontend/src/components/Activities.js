...
function DetailModal({ show, onClose, item }) {
  ...
  return (
    <>
      ...
      <div className="modal-custom d-block" tabIndex="-1">
              ...
              <table className="table table-striped table-hover">
                ...
                <tbody>
                  {items.map((it, idx) => (
                    <tr key={it.id || idx}>
                      <td>{it.id || idx + 1}</td>
                      <td>{it.name || it.title || it.username || '-'}</td>
                      <td className="table-json">{JSON.stringify(it).slice(0, 200)}</td>
                      <td className="actions-col">
                        <button className="btn btn-sm btn-primary" onClick={() => openDetail(it)}>View</button>
                        <a className="btn btn-sm btn-outline-secondary" href="#" onClick={(e)=>e.preventDefault()}>Link</a>
                      </td>
                    </tr>
                  ))}
                </tbody>
